...
/*
 * The renewals half of this module is deliberately absent.
 *
 * An earlier version derived review-due dates from the documents themselves.
 * `lib/maintenance` now holds that ground, from dates the customer enters,
 * and it feeds the dashboard directly. Two systems computing when a document
 * is due would disagree eventually, and the one the customer edits is the one
 * to keep.
 */
