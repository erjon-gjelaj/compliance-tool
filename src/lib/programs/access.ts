 ...
 * capability since it was written, `lib/pricing` maps every one-time offer to
 * it, and `FREE_INCLUDES` deliberately lists the gap check and not the
 * documents. Meanwhile `can()` was never called anywhere in the application.
 *
...
/**
 * What to say to somebody who cannot prepare this yet, and what it costs.
 *
 * The price comes from lib/pricing so that no screen carries a number, and it
 * is a range with `PRICING_NOTE` attached — see that module for why a single
 * figure would read as a quote.
 */
