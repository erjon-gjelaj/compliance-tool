 ...
 *
 * The real programs only break there when their prose happens to be the right
 * length, so testing on one of them is testing on a coincidence — the Hazard
 * Communication program passed this check while the bug was live, because none
 * of its blocks straddled a boundary. The filler is sized to push the list
 * across the join every time.
 */
