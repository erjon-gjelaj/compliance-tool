...
/**
 * Sets a company's plan. The operator console is the only caller.
 *
 ...
 * task. Every entitlement check already goes through `can()`, so this one
 * update is the whole of granting access.
 *
 * Matched on email rather than id because that is the identity this product
 * uses everywhere, and because the operator is looking at a request, which
 * carries an address and may carry no company id at all.
 *
 * Returns whether a row was actually changed, so the caller can tell "granted"
 * apart from "there is no company profile for that address yet" — silently
 * doing nothing is how an operator ends up telling a customer they have access
 * they do not have.
 */
